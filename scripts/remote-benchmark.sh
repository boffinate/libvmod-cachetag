...
Environment:
  ...
  CACHE_TAG_BENCH_PERF_STAT
                           Pass BENCH_PERF_STAT to the benchmark container for
                           opt-in warm-phase counter collection; forces
                           PERF_MODE=off so only one profiler attaches
                           (default: empty)
  CACHE_TAG_BENCH_PERF_STAT_RUNS
                           Number of repetitions to count, or all
                           (default: benchmark default, all)
  CACHE_TAG_BENCH_PERF_STAT_WORKLOAD
                           Optional workload basename to count (default: empty)
  CACHE_TAG_BENCH_PERF_STAT_EVENTS
                           perf stat event list (default: benchmark default)
  CACHE_TAG_BENCH_STALE_DELIVER
                           Pass BENCH_STALE_DELIVER to the benchmark container:
                           1 for the two-call vcl_hit + vcl_deliver shape, 0 for
                           the one-call shape (default: benchmark default)
...
bench_perf_stat_runs_override=${CACHE_TAG_BENCH_PERF_STAT_RUNS:-}
bench_perf_stat_workload_override=${CACHE_TAG_BENCH_PERF_STAT_WORKLOAD:-}
bench_perf_stat_events_override=${CACHE_TAG_BENCH_PERF_STAT_EVENTS:-}
bench_stale_deliver_override=${CACHE_TAG_BENCH_STALE_DELIVER:-}
...
if [ -n "\$CACHE_TAG_BENCH_PERF_STAT" ]; then
	...
	# perf stat and PERF_MODE both attach inherited counters to the same
	# vinyld. Keep remote perf-stat rows single-profiler, exactly as the
	# perf-record rows are.
	envs="\$envs PERF_MODE=off"
fi
if [ -n "\$CACHE_TAG_BENCH_PERF_STAT_RUNS" ]; then
	export BENCH_PERF_STAT_RUNS="\$CACHE_TAG_BENCH_PERF_STAT_RUNS"
fi
if [ -n "\$CACHE_TAG_BENCH_PERF_STAT_WORKLOAD" ]; then
	export BENCH_PERF_STAT_WORKLOAD="\$CACHE_TAG_BENCH_PERF_STAT_WORKLOAD"
fi
if [ -n "\$CACHE_TAG_BENCH_PERF_STAT_EVENTS" ]; then
	export BENCH_PERF_STAT_EVENTS="\$CACHE_TAG_BENCH_PERF_STAT_EVENTS"
fi
if [ -n "\$CACHE_TAG_BENCH_STALE_DELIVER" ]; then
	export BENCH_STALE_DELIVER="\$CACHE_TAG_BENCH_STALE_DELIVER"
fi
