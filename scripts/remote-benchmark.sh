...
Environment:
  ...
  CACHE_TAG_VINYL_THREADS  Vinyl worker thread cap. Defaults to logical CPUs,
                           clamped to at least CACHE_TAG_BENCH_CLIENTS.
  ...
  CACHE_TAG_BENCH_SET_INTERNING
                           1 to build cachetag with --enable-set-interning,
                           0 for the direct-vector baseline (default: 0)
  ...
  CACHE_TAG_BENCH_PERF_STAT
                           required for acknowledged load-phase cache-process
                           perf stat, empty/off to disable
  CACHE_TAG_BENCH_PERF_STAT_EVENTS
                           Comma-separated perf stat event list
  CACHE_TAG_BENCH_PERF_STAT_TIMEOUT
                           Phase and acknowledgement timeout seconds
...
vinyl_threads_override=${CACHE_TAG_VINYL_THREADS:-}
...
set_interning_override=${CACHE_TAG_BENCH_SET_INTERNING:-}
...
bench_perf_stat_override=${CACHE_TAG_BENCH_PERF_STAT:-}
bench_perf_stat_events_override=${CACHE_TAG_BENCH_PERF_STAT_EVENTS:-}
bench_perf_stat_timeout_override=${CACHE_TAG_BENCH_PERF_STAT_TIMEOUT:-}
...
if [ -n "\$CACHE_TAG_VINYL_THREADS" ]; then
	bench_vinyl_threads=\$CACHE_TAG_VINYL_THREADS
else
	bench_vinyl_threads=\$cores
	if [ "\$bench_vinyl_threads" -lt "\$bench_clients" ]; then
		bench_vinyl_threads=\$bench_clients
	fi
	if [ "\$bench_vinyl_threads" -lt 1 ]; then
		bench_vinyl_threads=1
	fi
fi
...
if [ -n "\$CACHE_TAG_BENCH_SET_INTERNING" ]; then
	envs="BENCH_SET_INTERNING=\$CACHE_TAG_BENCH_SET_INTERNING \$envs"
fi
envs="CACHE_TAG_ALLOW_STALE_BUILD=\$CACHE_TAG_ALLOW_STALE_BUILD \$envs"
envs="BENCH_VINYL_THREADS=\$bench_vinyl_threads \$envs"
...
"\$remote_dir/libvmod-cachetag/benchmarks/capture_system_metadata.sh" "\$result_dir/host-system.env"
{
	...
	printf 'bench_vinyl_threads_auto=%s\n' "\$bench_vinyl_threads"
	...
	printf 'bench_set_interning_override=%s\n' "\$CACHE_TAG_BENCH_SET_INTERNING"
...
if [ -n "\$CACHE_TAG_BENCH_PERF_STAT" ]; then
	export BENCH_PERF_STAT="\$CACHE_TAG_BENCH_PERF_STAT"
fi
if [ -n "\$CACHE_TAG_BENCH_PERF_STAT_EVENTS" ]; then
	export BENCH_PERF_STAT_EVENTS="\$CACHE_TAG_BENCH_PERF_STAT_EVENTS"
fi
if [ -n "\$CACHE_TAG_BENCH_PERF_STAT_TIMEOUT" ]; then
	export BENCH_PERF_STAT_TIMEOUT="\$CACHE_TAG_BENCH_PERF_STAT_TIMEOUT"
fi
