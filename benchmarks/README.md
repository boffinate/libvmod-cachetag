...
- `BENCH_CODE_GENERATION`: `runtime` (the candidate API, default) or `legacy` (a frozen compile-time comparison build). Runtime rows use `BENCH_RUNTIME_SET_INTERNING=0|1`, which is rendered into each namespace constructor. Legacy rows require `BENCH_LEGACY_SET_INTERNING=0|1` and retain the old configure flag only for that explicitly frozen build. Metadata records requested, rendered and effective modes; the decision contract also records the rendered workload hash.
...
- `BENCH_STORAGE_KIND`: `default` for the normal benchmark storage path, `fellow` for a patched Slash/Fellow storage lane, or `buddy` for a Slash/Buddy storage lane with only the current-Vinyl header compatibility patch.
...
Use `scripts/remote-benchmark.sh` for rented Debian/Ubuntu servers. The SSH target is the normal `user@host` argument or an entry from `~/.ssh/config`. The default remote workspace is `cachetag-bench` under the SSH user's home. Root SSH works, and cloud-image users such as `ubuntu` work when they have passwordless sudo; the remote runner defaults `REMOTE_DOCKER=auto`, which tries plain `docker` first and then `sudo -n docker`. It caps remote driver clients at 8 by default, fixes `BENCH_VINYL_THREAD_POOLS=2`, and sets the per-pool maximum from `CACHE_TAG_VINYL_THREAD_POOL_MAX` (or logical CPUs when unset), independently of the selected client count. It passes `--cap-add PERFMON` to Docker for hardware perf counters.
...
- `CACHE_TAG_VINYL_THREAD_POOL_MAX`: maximum Vinyl workers in each of the remote harness's two explicit pools; empty means logical CPUs and never follows `CACHE_TAG_BENCH_CLIENTS`.
...
`runtime-interning-decision-v1` is the fail-closed four-arm screen. Its remote matrix is ordered `D1, R0-1, I1, R1-1, D2, R0-2, I2, R1-2`; set `CACHE_TAG_LEGACY_DIRECT_VMOD_SRC`, `CACHE_TAG_LEGACY_INTERNED_VMOD_SRC`, and `CACHE_TAG_VMOD_BUILD_SRC` to three distinct frozen checkouts before syncing. D/I and R0/R1 each reuse only their immutable arm cache; a missing cache builds the first row, while later rows force provenance-checked reuse. The controlled profiler attaches `perf` to the exact non-empty cache-main TID list captured at phase start and records that list with matching start/end counts. Because `perf` does not expose an independent post-attachment TID map, a task-set change is treated as uncovered work and rejects the phase. The summarizer also requires all eight labels in order, task coverage, rendered mode, decision-cohort identity, and candidate-binary provenance.

Before starting that group, set all four non-negative, pre-frozen dispersion budgets: `CACHE_TAG_DECISION_SHARED_LOAD_BUDGET_PERCENT`, `CACHE_TAG_DECISION_SHARED_WARM_BUDGET_PERCENT`, `CACHE_TAG_DECISION_UNIQUE_LOAD_BUDGET_PERCENT`, and `CACHE_TAG_DECISION_UNIQUE_WARM_BUDGET_PERCENT`. The controller refuses to start without them. D1 and I1 must each contain exactly three eligible repetitions of both shapes and remain within their matching load and warm budget before the first R0 or R1 row is allowed.

Slash-backed storage benchmarks are deliberately isolated from the default in-memory matrices. Use `BENCH_STORAGE_KIND=fellow` or the dedicated `fellow-*` remote matrices when the question is Fellow-backed persistent storage behavior; these lanes build patched Slash/Fellow inside the benchmark container and run cachetag with a persistent namespace by default. Use `BENCH_STORAGE_KIND=buddy` or the dedicated `buddy-*` remote matrices when the question is Buddy-backed volatile storage behavior; Buddy lanes build Slash/Buddy with only the current-Vinyl header compatibility patch, keep cachetag persistence disabled by default and do not carry the Fellow shutdown, log or cachetag patches.
